        ...
        if ok_destinatario:
            ...
            if ok_data_saida:
                try:
                    ...
                    cursor.execute(
                        "UPDATE produtos SET quantidade = %s, data_saida = %s, destinatario = %s WHERE id = %s",
                        (nova_quantidade, data_saida_formatada, destinatario, valor_id),
                    )
                    conn.commit()
                    listar_produtos()
                    print(f"{quantidade_saida} unidades foram retiradas do estoque.")
                except ValueError:
                    print("Data de saída inválida ou formato incorreto.")
            else:
...
def editar_produtos():
    ...
    tela_editar.lineEdit_9.setText(str(produto[8]))
...
def salvar_edicao():
    ...
    data_saida = tela_editar.lineEdit_8.text()
    destinatario = tela_editar.lineEdit_9.text()

    cursor = conn.cursor()
    cursor.execute(
        "UPDATE produtos SET produto = '{}', descricao = '{}', preco = '{}', categoria ='{}', data = '{}', quantidade = '{}', data_saida = '{}', destinatario ='{}' WHERE id = {}".format(
            produto,
            descricao,
            preco,
            categoria,
...
def listar_produtos():
    ...
    tela_lista.tableWidget.setColumnCount(9)  # Ajuste o número de colunas

    for i in range(0, len(dados_lidos)):
        for j in range(0, 9):  # Ajuste o número de colunas
...
tela_saida = uic.loadUi(caminho_editar_produto_ui)
...
tela_lista.pushButton_5.clicked.connect()
